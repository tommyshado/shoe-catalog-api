...
async function fetchCartItems() {
  ...
  const userId = user; // Ensure 'user' is defined and has a value
  console.log(userId)
  const response = await fetch(`/api/cart/${userId}`);
  const cartItems = await response.json();
  
  console.log('Received cart items:', cartItems); // Debug line
...
async function updateCartUI() {
  console.log('Updating cart UI...'); // Debug line
  let cartItems = Object.values(cart);
  
  console.log('Cart items for UI:', cartItems); // Debug line
  ...
  if (cartTemplate) {
    ...
    console.log('Generated HTML:', generatedHTML); // Debug line
    
    let cartItemsContainer = document.querySelector(".cart_items");
    if (cartItemsContainer) {
      cartItemsContainer.innerHTML = generatedHTML;
    } else {
      console.log('Cart items container not found'); // Debug line
    }
  } else {
    console.log('Cart template not found'); // Debug line
  }
}
