...
async function updateCartQuantity(cart_id, newQuantity) {
  console.log(`About to update cart. Cart ID: ${cart_id}, New Quantity: ${newQuantity}`);
  try {
      const result = await db.none('UPDATE "public"."carts" SET quantity = $1 WHERE cart_id = $2', [newQuantity, cart_id]);
      console.log("Successfully updated cart: ", result);
      return result;
  } catch (err) {
